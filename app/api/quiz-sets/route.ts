...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase
      .from('quiz_sets')
      .insert({ name, description, subject, grade })
      .select()
      .single();
...
export async function PUT(request: Request) {
  try {
    ...
    const body = await request.json();
    const { id, name, description, subject, grade, is_active } = body;
    ...
    const { data, error } = await supabase
      .from('quiz_sets')
      .update({ name, description, subject, grade, is_active, updated_at: new Date().toISOString() })
      .eq('id', id)
      .select()
      .single();
